...
__global__ void KNN(float *train, float *test, int *predictions, float *candidates, int *classCounts, int *datasetSizes , int k, int numAttr, int numClasses) {
    ...
    if(tid < datasetSizes[1]) {
        // printf("(%d)\n", tid);
        // printf("(%f, %f), (%f, %f)\n", test[0], train[0], test[1], train[1]);
        // printf("%d", predictions[0]);
        // printf("%d, %d", datasetSizes[0], datasetSizes[1]);
        // printf("{%d, %d}\n", numAttr, numClasses);
        // printf("[%f, %d]\n", candidates[0], classCounts[0]);

        // int queryIndex = tid * numAttr;
        ...
        for (int keyIndex = 0; keyIndex < datasetSizes[0] * numAttr; keyIndex += numAttr) {
            ...
            for (int c = 0; c < k; c++) {
                if (dist < candidates[candidateForTid + 2 * c]) {
                    ...
                    // printf("%d, %d: %f, %f\n", tid, keyIndex/8, candidates[candidateForTid + 2 * c], candidates[candidateForTid + 2 * c + 1]);
                    // printf("%d, %d: %f, %f\n", tid, keyIndex/8, train[keyIndex], train[keyIndex + numAttr - 1]);
